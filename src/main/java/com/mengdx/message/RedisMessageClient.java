...
@Component
@Slf4j
public class RedisMessageClient<T> {
     ...
     * @return
     */
    public boolean pushListMessage(RedisMessage<T> message) {
     ...
     * @return 返回结果
     */
    public boolean pushSetMessage(RedisMessage<T> message) {
     ...
     * @return 返回结果
     */
    public boolean pushQueueMessage(RedisMessage<T> message) {
        redisTemplate.convertAndSend(message.getTopic(), message.getData());
        log.info("发送订阅方式的消息成功");
        return true;
    }

    public boolean pushStreamMessage(RedisMessage<T> message) {
        redisTemplate.convertAndSend(message.getTopic(), message.getData());
        return true;
    }
